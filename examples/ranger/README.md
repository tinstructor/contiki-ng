...
>**Note:** a basic python script is also provided to analyze the logfile. The script may be called by providing the name of the logfile to be analyzed as well as the csv file to whic log-derived information must be appended: `$ python3 analyzer-ng.py <name of logfile>.log <name of csv file>.csv`. However, this script will undergo significant changes in the future to incorporate more advanced analysis of additional metrics etc., so don't expect it to work flawlessly.
